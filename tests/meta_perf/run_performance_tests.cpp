...
void run_manifest_entries(std::shared_ptr<arrow::fs::FileSystem> fs, const int times, bool use_avro_reader_schema,
                          const uint32_t threads_num, const iceberg::ice_tea::GetScanMetadataConfig& config) {
...
=======
        [&](iceberg::Schema& schema) { return use_avro_reader_schema; }, nullptr, config);
>>>>>>> fd541a2 (v1)
...
ABSL_FLAG(uint32_t, threads_num, 0, "");
...
int main(int argc, char* argv[]) {
  ...
  const uint32_t threads_num = absl::GetFlag(FLAGS_threads_num);
  ...
  run_manifest_entries(fs, manifest_entries, use_avro_reader_schema, threads_num, config);
  run_manifest_entries_wide(fs, manifest_entries_wide, use_avro_reader_schema, threads_num, config);
  run_manifest_files(fs, manifest_files, use_avro_reader_schema, threads_num, config);
  run_schemas(fs, schemas, use_avro_reader_schema, threads_num, config);
  run_snapshots(fs, snapshots, use_avro_reader_schema, threads_num, config);
