...
ManifestEntry MakePositionalDeleteEntryUnannotated(const std::string& delete_path) {
  ManifestEntry entry;
  entry.status = ManifestEntry::Status::kAdded;
  entry.data_file.content = ContentFile::FileContent::kPositionDeletes;
  entry.data_file.file_format = "PARQUET";
...
TEST(ReferencedDataFile, GetScanMetadataUnannotatedDeleteOptimizationDisabled) {
  ScopedTempDir dir;

  {
    Manifest manifest;
    {
      ...
      manifest.entries.emplace_back(MakePositionalDeleteEntry("delete1", "data1"));
      manifest.entries.emplace_back(MakePositionalDeleteEntry("delete2", "data2"));
      manifest.entries.emplace_back(MakePositionalDeleteEntryUnannotated("delete3"));
    }
