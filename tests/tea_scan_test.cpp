...
TEST(GetScanMetadata, WithPartitionSpecs) {
  ...
  std::vector<TestInfo> path_to_expected_partitions_count = {
      ...
      {"s3://warehouse/day_timestamptz_partitioning/metadata/00002-bf5ed300-c344-41fe-87ad-0d1190705bf9.metadata.json",
       3, 3, 0},
      {"s3://warehouse/w_clean_expired_metadata/metadata/00005-6d3c29aa-bb3a-45a8-adb6-51c2ab74195b.metadata.json", 3,
       3, 0},
...
TEST(GetScanMetadata, DanglingPositionalDeletes) {
  ...
  for (bool use_avro_reader_schema : {false, true}) {
    ice_tea::GetScanMetadataConfig config;
    if (use_avro_reader_schema) {
      config.manifest_entry_deserializer_config.datafile_config.extract_lower_bounds = false;
      config.manifest_entry_deserializer_config.datafile_config.extract_upper_bounds = false;
    }
