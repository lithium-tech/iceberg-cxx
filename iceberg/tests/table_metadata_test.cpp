...
TEST(Metadata, SanityCheck) {
  std::ifstream input(
      "metadata/00003-aaa5649c-d0a0-4bdd-bf89-1a63bba01b37.metadata.json");

  std::stringstream ss;
  ss << input.rdbuf();
  std::string data = ss.str();

  const TableMetadataV2 metadata = MakeTableMetadataV2(data);
  ...
  ASSERT_EQ(metadata.schemas[0].GetFields().size(), 2);
  auto& field = metadata.schemas[0].GetFields()[1];
  EXPECT_EQ(field.id, 2);
  EXPECT_EQ(field.is_required, false);
  EXPECT_EQ(field.name, "b");
  ASSERT_EQ(field.type->IsPrimitive(), true);
  EXPECT_EQ(field.type->Id(), iceberg::Type::kLong);
  ...
  std::map<std::string, std::string> expected_properties = {
      {"created-at", "2024-03-20T12:47:31.109851401Z"},
      {"owner", "root"},
  ...
  std::map<std::string, std::string> expected_summary_for_snapshot_0 = {
      {"added-data-files", "6"},       {"added-files-size", "25206"},
      {"added-records", "10000"},      {"changed-partition-count", "1"},
      {"operation", "append"},         {"spark.app.id", "local-1710938745155"},
      {"total-data-files", "6"},       {"total-delete-files", "0"},
      {"total-equality-deletes", "0"}, {"total-files-size", "25206"},
      {"total-position-deletes", "0"}, {"total-records", "10000"}};
  ...
  std::vector<std::pair<int64_t, int64_t>> expected_snapshot_log = {
      {1710938855718, 2084304437907955247},
      {1710938891838, 2635333433439510679},
      {1710939086483, 765518724043979080}};
