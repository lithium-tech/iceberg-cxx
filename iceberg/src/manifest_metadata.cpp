...
std::vector<ManifestMetadata> MakeManifestList(const std::string& data) {
  ...
  while (data_file_reader.read(manifest_file)) {
    ...
    manifest.added_data_files_count = manifest_file.added_data_files_count;
    manifest.existing_data_files_count =
        manifest_file.existing_data_files_count;
    manifest.deleted_data_files_count = manifest_file.deleted_data_files_count;
