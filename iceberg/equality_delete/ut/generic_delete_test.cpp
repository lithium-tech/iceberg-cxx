...
TEST(GenericDeleteTest, Null) {
  ...
  ASSERT_OK(del.Add({del_array}, 1, 0));
  EXPECT_EQ(del.Size(), 1);

  auto check_array = CreateArray<arrow::Int32Builder>(OptionalVector<int32_t>{std::nullopt, 1, 2, 12});
  EXPECT_EQ(del.IsDeleted({check_array}, 0, 0), true);
  EXPECT_EQ(del.IsDeleted({check_array}, 1, 0), false);
  EXPECT_EQ(del.IsDeleted({check_array}, 2, 0), false);
  EXPECT_EQ(del.IsDeleted({check_array}, 3, 0), false);
}
...
TEST(SpecializedMultipleColumnDelete, Nulls) {
  ...
  ASSERT_OK(del.Add({del_array1, del_array2}, 3, 0));
...
TEST(SpecializedMultipleColumnDelete, VariableLength) {
  ...
  ASSERT_OK(del.Add({del_array1}, 4, 0));
  ...
  EXPECT_EQ(del.IsDeleted(check_arrays, 0, 0), true);
  ...
  EXPECT_EQ(del.IsDeleted(check_arrays, 5, 0), true);
  EXPECT_EQ(del.IsDeleted(check_arrays, 6, 0), false);
  EXPECT_EQ(del.IsDeleted(check_arrays, 7, 0), false);
}
...
TEST(SpecializedMultipleColumnDelete, VariableLengthMultipleColumns) {
  ...
  ASSERT_OK(del.Add({del_array1, del_array2}, 1, 0));
  EXPECT_EQ(del.Size(), 1);
